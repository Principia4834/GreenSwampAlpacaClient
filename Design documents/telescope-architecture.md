...
**Get Well Guidance.md Stage 2 amendment:** §6.4's "supplementary, opportunistic" framing and §6.5's "prefers SignalR-pushed updates when fresh; falls back to REST-polled updates when SignalR is stale/absent" reconciliation description are **superseded for GreenSwamp-class telescopes** by Stage 2 of `Get Well Guidance.md` — see the notes inline in §6.4/§6.5 below, and `SignalR-transport-implementation-plan.md` Decision B/D (RESOLVED), for the confirmed model: SignalR is the single source of truth for all state fields while connected, with REST/AP used only after an irrecoverable loss (5 failed reconnects at 2/5/10/10/10s intervals). This document's other sections are unaffected by Stage 2 and have not been touched.

**Get Well Guidance.md Stage 3 amendment:** §6.3's "to-be-confirmed" poll cadence and §11's "Poll cadence and reconnect/backoff policy constants" open item are now **RESOLVED** by Stage 3 of `Get Well Guidance.md` — see the note inline in §6.3 below, and `SignalR-transport-implementation-plan.md` Decision C (RESOLVED), for the confirmed cadences: a 250ms SignalR loop for GreenSwamp-class telescopes, and a 1 second ASCOM Alpaca `DeviceState` loop for standard Alpaca-class telescopes (and as the GreenSwamp-class fallback loop). Reconnect/backoff policy constants remain fixed by Stage 2 (5 retries at 2/5/10/10/10s); internal timer/thread-safety mechanics remain Stage 4 work.

**Get Well Guidance.md Stage 4 amendment:** §6.6's "no built-in back-pressure or coalescing" limitation (item 1) and §11's "Reconciliation algorithm details (FR43)" open item are now **RESOLVED** by Stage 4 of `Get Well Guidance.md` — see the notes inline in §6.5/§6.6 below, and `SignalR-transport-implementation-plan.md` Decision C/D (Stage 4, RESOLVED) and Finding A4 (RESOLVED), which confirm that resolving Telescope State (Stage 2) and Poll Loop (Stage 3) removes all remaining back-pressure/reconciliation concerns: there is no transition-boundary staleness (SignalR and the fallback loop are never concurrent active sources), no cross-thread signaling problem (`TelescopeSession` owns all transport concerns end-to-end regardless of thread), and the only multi-cadence concurrency case (AlpacaClass polling) is resolved by time-ordering/sequencing the fixed-cadence blocking REST calls. This document's other sections are unaffected by Stage 4 and have not been touched.
...
- **Position/state polling loop**: uses an injected `TimeProvider` (NFR55-57, test strategy §8) rather than `Task.Delay` directly, so Level 1 tests can advance a `FakeTimeProvider` deterministically. Poll cadence is a to-be-confirmed constant/setting (Open Item), default informed by the reference server's own 250ms internal refresh (`TelescopeStateService`) as a reasonable starting point, not a hard requirement.
  - **Get Well Guidance.md Stage 3 amendment:** the cadence is no longer to-be-confirmed. Per Stage 3 of `Get Well Guidance.md` (see `SignalR-transport-implementation-plan.md` Decision C, RESOLVED), this `AlpacaTelescopeProvider`/ASCOM Alpaca `DeviceState` poll loop runs at a fixed **1 second** cadence — used continuously for standard Alpaca-class telescopes, and as the GreenSwamp-class Decision D fallback loop only (engaged after irrecoverable SignalR loss). The separate GreenSwamp SignalR ingestion loop (§6.4) runs at a fixed **250ms** cadence instead, matching the reference server's own internal tick referenced above (which is no longer merely a "reasonable starting point" but the confirmed cadence).
...
**Get Well Guidance.md Stage 2 amendment:** the heading above ("supplementary, opportunistic") and the standing assumption below that this is merely a lower-latency alternative to REST data no longer describe the confirmed state-sourcing model for GreenSwamp-class telescopes. Per Stage 2 of `Get Well Guidance.md` (see `SignalR-transport-implementation-plan.md` Decision B/D, RESOLVED), once the hub described here exists, `GreenSwampSignalRProvider` becomes the **single source of truth for all telescope state fields** for a GreenSwamp-class device, not a supplementary/opportunistic add-on to REST polling — `AlpacaTelescopeProvider`-sourced state is used for this class only as a fallback after an irrecoverable SignalR loss (5 failed reconnects at 2/5/10/10/10s intervals), not continuously alongside SignalR. Command dispatch remains REST-only and out of scope for SignalR regardless (unaffected by this amendment).
...
- Implements FR41-43 reconciliation. **Get Well Guidance.md Stage 2 amendment (supersedes the bullet below for GreenSwamp-class devices):** for a GreenSwamp-class device, SignalR is the single source of truth for all state fields while connected — there is no continuous "prefer SignalR if fresh, else REST" merge for this class; `TelescopeSession` switches to REST/AP-sourced state only after 5 failed SignalR reconnect attempts at 2/5/10/10/10s intervals (irrecoverable loss), and switches back once reconnected. For a standard Alpaca-class device (no SignalR source at all), state is always REST/AP-sourced per the cadences fixed in `SignalR-transport-implementation-plan.md` Decision B. The original bullet below ("prefers SignalR-pushed updates when fresh; falls back to REST-polled updates when SignalR is stale/absent") described a continuous freshness-based merge model that Stage 2 does not confirm; it is retained here only for traceability of the architecture's evolution, not as the current design:
  - *(original, superseded)* prefers SignalR-pushed updates when fresh; falls back to REST-polled updates when SignalR is stale/absent; de-duplicates/orders by a monotonic sequence or `TimeProvider`-derived timestamp attached at ingestion (not the transport's own wall-clock, to avoid clock-skew issues between client and two different transports/servers). Exact reconciliation algorithm remains an Open Item (per requirements §12, test strategy §12) but the seam (a single ingestion point per instance, both providers feed into it, not the ViewModel) is fixed here.
  - **Get Well Guidance.md Stage 4 amendment — RESOLVED:** the "exact reconciliation algorithm" concern above is closed, not merely narrowed. Since SignalR and the fallback loop are never concurrent active sources for a GreenSwamp-class device (Stage 2/Decision D), there is no freshness-based merge algorithm to design at all — `TelescopeSession` simply publishes whichever single source is currently active. For a standard Alpaca-class device, the only reconciliation need is time-ordering/sequencing the multiple fixed-cadence blocking REST calls (Decision B/C) so each field updates the model as its own call completes; standard OnChange semantics then apply per field. See `SignalR-transport-implementation-plan.md` Decision C/D (Stage 4, RESOLVED) for the full disposition.
...
1. **No built-in back-pressure or coalescing.** If `AlpacaTelescopeProvider`'s poll loop and a future SignalR provider both raise `PositionUpdated` in quick succession, subscribers (ViewModels) receive every raised event, at whatever rate providers produce them. Mitigation: `TelescopeSession`'s reconciliation layer (§6.5) is responsible for **not raising** redundant/stale events in the first place (FR43) — the event mechanism itself does no filtering, so correctness here rests entirely on the orchestrator, not the transport. **Get Well Guidance.md Stage 4 amendment — RESOLVED:** this concern is closed for the confirmed design, not merely mitigated. The scenario of "both a REST-poll and a SignalR provider raising events in quick succession for the same instance" does not actually arise — SignalR and the fallback REST/AP loop are never concurrent active sources for a GreenSwamp-class device (§6.5, Stage 4 amendment) — so there is no dual-source event storm to coalesce against. The only remaining rapid-succession case is an AlpacaClass session's multiple fixed-cadence REST calls, which is resolved by time-ordering/sequencing those calls rather than by event-level coalescing (see `SignalR-transport-implementation-plan.md` Decision C, Stage 4).
...
- ✅ **Reconciliation algorithm details (FR43)** — **RESOLVED (Get Well Guidance.md Stage 4)**: the merge/staleness/ordering question this item originally deferred ("not yet relevant, no second source exists yet") is now closed, not just newly relevant. Per Stage 4's confirmed assertion (see `SignalR-transport-implementation-plan.md` Decision C/D, Stage 4, RESOLVED, and Finding A4, RESOLVED): there is no dual-source merge algorithm needed at all, because SignalR and the Decision D fallback loop are never concurrent active sources for a GreenSwamp-class device — resolving Telescope State (Stage 2) and Poll Loop (Stage 3) removes the reconciliation problem this item anticipated. The only remaining ordering concern (AlpacaClass telescopes' multiple fixed-cadence REST calls) is resolved by time-ordering/sequencing those calls, with standard OnChange semantics applying per field — *(original open item, superseded, retained for traceability)*: the merge/staleness/ordering algorithm inside `TelescopeSession` is specified here only as "must exist as a single ingestion point," not algorithmically. **Not yet relevant** — no second (SignalR) source exists yet to reconcile against.
...
- ✅ **Poll cadence and reconnect/backoff policy constants** — **RESOLVED (Get Well Guidance.md Stage 3, poll cadence) and Stage 2 (reconnect/backoff policy)**: a continuous background polling loop is now required, at fixed cadences — a 250ms SignalR ingestion loop for GreenSwamp-class telescopes while connected, and a 1 second ASCOM Alpaca `DeviceState` loop for standard Alpaca-class telescopes (used continuously) and GreenSwamp-class telescopes (used only as the Decision D fallback, engaged after 5 failed reconnects at the 2/5/10/10/10s reconnect/backoff schedule already fixed by Stage 2). This closes requirements FR37/38/41's "internal polling mechanism"/"ongoing updates" gap for both classes at the cadence-decision level (see `SignalR-transport-implementation-plan.md` Decision C, RESOLVED). Internal timer composition and thread-safety of starting/stopping the fallback loop are now also **RESOLVED (Get Well Guidance.md Stage 4)** — see `SignalR-transport-implementation-plan.md` Decision C/D (Stage 4, RESOLVED): no bespoke composition/locking scheme is required, only time-ordering/sequencing of the AlpacaClass multi-cadence REST calls and reliance on SignalR/fallback mutual exclusivity — *(original open item, superseded, retained for traceability)*: the current implementation has **no continuous background polling loop at all**. `TelescopeSession` publishes a `StateUpdated` snapshot only (a) once immediately after a successful `ConnectAsync`, and (b) once immediately after each of `FindHome`/`ParkAsync`/`AbortSlewAsync` completes. This satisfies the four in-scope commands' own "resulting state" needs (implementation design §6.1-6.4) but does **not** yet satisfy requirements FR37/38/41 ("shall provide *ongoing* telescope position/state updates" / "shall obtain updates via an internal polling mechanism") for a connected-but-idle session. A periodic poll loop (using the already-injected `TimeProvider`, consistent with this section's original intent) is required before FR37/38/41 can be considered met, and is the most likely next piece of Model-layer work.
