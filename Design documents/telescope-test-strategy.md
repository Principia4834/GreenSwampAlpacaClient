...
- Finalize the exact shape of the application-facing telescope abstraction so Level 2 fakes can be written against a stable contract.
- Decide the concrete time-abstraction mechanism (e.g. `TimeProvider` vs custom interface) to be used across providers.
- Decide how "GreenSwamp-class" capability detection works, so Level 1/3 tests can simulate/verify both positive and negative detection cases.
- Decide the reconciliation algorithm for combined update streams, so Level 1 tests can be written test-first alongside its implementation, then confirmed at Level 3 against the real GreenSwamp server.
- Confirm exact startup flags/configuration needed to run the ASCOM reference simulator and GreenSwampAlpacaServer headlessly, on isolated ports, with isolated settings, for automated test runs (Section 6, Process Lifecycle Manager).
- Select or build the fault/latency-injecting proxy component and confirm it can sit in front of both a plain HTTP (Alpaca REST) and a SignalR (WebSocket-based) connection.
- Decide how test doubles for the SignalR command surface (not yet implemented server-side) should be shaped, given that surface does not exist yet in `GreenSwampAlpacaServer`, and how/when the temporary fake hub is retired once real support lands.
- Decide the acceptable test-run duration/frequency trade-off between the fast Level 1/2 unit suite and the slower, real-process-backed Level 3-6 integration suite (e.g. local dev loop vs PR gate).
