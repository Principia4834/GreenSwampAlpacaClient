...
- **FR14 detection trigger is now resolved (§6.8); the exact match/allow-list strategy is not.** Whether the `ServerName`/`Manufacturer` recognition rule is a hard-coded constant, a configurable allow-list, or a pattern match, and how it degrades on a near-miss (e.g. a future GreenSwamp server version renaming itself) needs a concrete decision at implementation-design stage.
- **Full hand-controller-parity jog/manual-control design is deferred (§6.7).** v1 ships `MoveAxis`-only jog. A dedicated future design pass must analyze, in the context of the current GreenSwampAlpacaServer solution: how (or whether) HC mode/flip/anti-backlash/hemisphere/pier-side logic should be exposed to this client at all; whether that requires new Alpaca REST surface, the future GreenSwamp SignalR command channel (§6.4), or remains a GreenSwampAlpacaServer-internal-only capability (Blazor UI only); and how tracking-rate composition (§6.7's `RateMovePrimaryAxis`/`RateMoveSecondaryAxis` reassert-tracking behavior) should be modeled client-side, if at all.
